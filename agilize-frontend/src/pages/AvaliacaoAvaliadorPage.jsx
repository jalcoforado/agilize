...
function FormularioAvaliador({ status, idDemanda, onSucesso, onErro }) {
  ...
  if (!isFase1 && !isFase3) {
    return (
      <div className="bg-neutral-50 border border-neutral-200 rounded-xl p-5 text-center">
        ...
        <p className="text-sm text-neutral-500">Esta demanda não está aguardando decisão do Avaliador Técnico.</p>
  ...
  const executar = async (acao) => {
    ...
    try {
      if (acao === 'solicitar-ajustes')
        await demandaService.avaliadorSolicitarAjustes(idDemanda, parecer, comentario, anexos);
      else if (acao === 'devolver-analista')
        await demandaService.avaliadorDevolverAnalista(idDemanda, parecer, comentario, anexos);
      onSucesso(acao);
    } catch (err) {
    ...
    <div className="bg-white rounded-xl border border-neutral-200 overflow-hidden">
      ...
      <div className="px-5 py-4 space-y-4">
        ...
        <p className="text-xs text-neutral-500 leading-relaxed bg-neutral-50 border border-neutral-200 rounded-lg px-3 py-2.5">
          Como Avaliador Técnico, você pode <strong>solicitar ajustes</strong> diretamente ao solicitante, ou <strong>devolver ao analista</strong> com orientações para que ele continue a análise.
        </p>
...
export default function AvaliacaoAvaliadorPage() {
  const { id } = useParams();
  ...
  const faseInfo = FASES_AVALIADOR[demanda?.status_atual];
