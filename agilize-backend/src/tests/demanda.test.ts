import { request, app, db, login } from './helpers';

let tokenSolicitante: string;
let idDemandaCriada: string | number | undefined;
...
describe('POST /api/v1/demandas — criar demanda', () => {
  ...
  test('cria demanda com campos de risco preenchidos', async () => {
    const res = await request(app)
      .post('/api/v1/demandas')
      .set('Authorization', `Bearer ${tokenSolicitante}`)
      .send({
        titulo: 'Teste com campos de risco e impacto',
        descricao: 'Demanda criada pelo Jest para validar os novos campos de avaliação de risco. Esta descrição tem mais de 50 caracteres.',
        tipo_solucao: 'SCRIPT',
        prioridade: 'MEDIA',
        id_unidade: 6,
        id_departamento: 9,
        objetivo_principal: 'Validar persistência dos campos de risco nos testes de integração da API.',
        publico_alvo: 'Equipe de QA',
        frequencia_uso: 'DIARIO',
        quantidade_usuarios_estimada: 2,
        solucao_em_uso: true,
        dados_sensiveis: true,
        dados_sensiveis_desc: 'CPF e e-mail dos usuários',
        impacta_outras_areas: true,
        areas_impactadas: 'Financeiro, Jurídico',
        usa_ia_desenvolvimento: false,
      });

    expect(res.status).toBe(201);
    expect(res.body.demanda.solucao_em_uso).toBe(true);
    expect(res.body.demanda.dados_sensiveis).toBe(true);
    expect(res.body.demanda.dados_sensiveis_desc).toBe('CPF e e-mail dos usuários');
    expect(res.body.demanda.impacta_outras_areas).toBe(true);
    expect(res.body.demanda.areas_impactadas).toBe('Financeiro, Jurídico');
    expect(res.body.demanda.usa_ia_desenvolvimento).toBe(false);

    // Limpar a demanda extra criada
    if (res.body.demanda?.id_demanda) {
      await db('tb_notificacoes').where('id_demanda', res.body.demanda.id_demanda).del().catch(() => {});
      await db('tb_historico_decisoes').where('id_demanda', res.body.demanda.id_demanda).del().catch(() => {});
      await db('tb_demandas').where('id_demanda', res.body.demanda.id_demanda).del().catch(() => {});
    }
  });
...
describe('GET /api/v1/demandas — listar', () => {
  ...
  test('filtro por status retorna apenas demandas do status', async () => {
    ...
    if (res.body.demandas.length > 0) {
      res.body.demandas.forEach((d: { status_atual: string }) => expect(d.status_atual).toBe('DRAFT'));
    }
  });
});
